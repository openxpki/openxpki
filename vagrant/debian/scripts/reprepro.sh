...
if [ -d "/code-repo/package/debian/extra" ]; then
    find /code-repo/package/debian/extra -name "*.deb" | xargs -L1 --no-run-if-empty reprepro --confdir  /packages.openxpki.org/$PACKAGE/conf/ includedeb $DIST;
fi
...
# Add the apt config
echo $SOURCE > /packages.openxpki.org/$PACKAGE/openxpki.list 
