...
}, 'Create workflow: certificate signing request')
...
my $cert_serial = $test->get_msg()->{PARAMS}->{BODY}->{SERIAL_HEX};
...
my $certs = $data->get_msg()->{PARAMS}->{LIST};
my $is_in_crl = scalar grep { Math::BigInt->from_hex($_->{SERIAL}) eq Math::BigInt->from_hex($cert_serial) } @$certs;
$test->ok($is_in_crl, 'Revoked certificate is included in CRL');
