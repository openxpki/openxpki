...
var SSC_MODEL = new Class(
	...
	//				}
			...
			sc_test_card: function(pin, viewCb) {
				...
				for(var i=0; i < this.user.parsedCerts.length ; i++ ){
					this.user.parsedCerts[i].VISUAL_STATUS = "red";
					//this.user.parsedCerts[i].MODULUS_HASH = "no key";
					this.user.parsedCerts[i].PRIVATE_KEY_TEST_RESULT = "FAIL_NO_KEY";
				}
				...
				for( var j=0 ; j < res.length-1 ; j++ ){
					...
					for(var i=0; i < this.user.parsedCerts.length ; i++ ){
						...
						//set staus to red incase the is no keypair to test for a certificate
						//this.user.parsedCerts[i].VISUAL_STATUS = "red";
						window.dbg.log("set red  "+ this.user.parsedCerts[i].MODULUS_HASH);
						
						if(key[0] === this.user.parsedCerts[i].MODULUS_HASH ){
							...
							if( PRIVATE_KEY_TEST_RESULTS[j].MODULUS_HASH === key[0]){
								...
								window.dbg.log("found cert for key :" +  PRIVATE_KEY_TEST_RESULTS[j].MODULUS_HASH + "  cert:" + this.user.parsedCerts[i].SUBJECT  );
							}
							
							this.user.parsedCerts[i].PRIVATE_KEY_TEST_RESULT = key[1];
							if(this.user.parsedCerts[i].PRIVATE_KEY_TEST_RESULT === "PASS"){
								window.dbg.log("test result pass : "+ this.user.parsedCerts[i].PRIVATE_KEY_TEST_RESULT );
								
								this.user.parsedCerts[i].VISUAL_STATUS = "green" ;
								window.dbg.log("VISUAL_STATUS PASS : "+ this.user.parsedCerts[i].VISUAL_STATUS );
							}else{
								window.dbg.log("VISUAL_STATUS FAILs : "+ this.user.parsedCerts[i].VISUAL_STATUS );
								this.user.parsedCerts[i].VISUAL_STATUS = "red" ;
							}			
						}	
					}
				...
				window.dbg.log(this.user.parsedCerts);
				window.dbg.log(PRIVATE_KEY_TEST_RESULTS );
...
//					}
				...
				case 'deactivated':
					...
					sscView.showPopUp('E_smartcard_deactivated', 'cross', 'E_deactivated_by_badgeoffice' );
					return;
					break;
				case 'activated':
					ret = 0;
