...
$mech->follow_link(text => 'I18N_OPENXPKI_HTML_MENU_CREATE_CSR', n => '1');
...
$mech->form_name('OpenXPKI');
$mech->field('cert_subject_hostname', 'example2.example.com');
...
$mech->field('role', 'Web Server');
$mech->click('__submit');
like($mech->response->content, qr/I18N_OPENXPKI_CLIENT_HTML_MASON_CREATE_CSR_GET_KEYTYPE_TITLE/, 'Private key type');

$mech->form_name('OpenXPKI');
$mech->field('additional_info_phone', '1234');
$mech->field('keygen', 'SERVERSIDE');
$mech->field('keytype', 'RSA');
$mech->field('additional_info_comment', 'comment');
$mech->field('cert_subject_hostname', 'example2.example.com');
$mech->field('cert_subject_port', '1234');
$mech->field('subject_style', '00_tls_basic_style');
$mech->field('role', 'Web Server');
$mech->click('__submit');
like($mech->response->content, qr/I18N_OPENXPKI_CLIENT_HTML_MASON_CREATE_CSR_GET_KEYPARAMETERS_TITLE/, 'Private key parameters');
...
$mech->field('cert_subject_port', '1234');
$mech->field('subject_style', '00_tls_basic_style');
$mech->field('PARAMETER_ENC_ALG', 'aes256');
$mech->field('PARAMETER_KEY_LENGTH', '4096');
$mech->field('role', 'Web Server');
$mech->click('__submit');
like($mech->response->content, qr/I18N_OPENXPKI_CLIENT_HTML_MASON_CREATE_CSR_GET_PASSWORD_GENERATION_METHOD_TITLE/, 'Password generation method');
...
$mech->field('PARAMETER_KEY_LENGTH', '4096');
$mech->field('role', 'Web Server');
$mech->field('password_gen', 'generate');
$mech->click('__submit');
like($mech->response->content, qr/I18N_OPENXPKI_CLIENT_HTML_MASON_CREATE_CSR_GET_PASSWORD_GENERATED_TITLE/, 'Generated password');
my ($password) = ($mech->response->content =~ m{ <b> (.+) </b> }xms);
if ($ENV{DEBUG}) {
...
like($mech->response->content, qr/CN=example2.example.com:1234,DC=Test\ Deployment,DC=OpenXPKI,DC=org/, 'Certificate subject');
like($mech->response->content, qr/-----BEGIN\ CERTIFICATE\ REQUEST-----/, 'PKCS#10 present');
like($mech->response->content, qr/-----BEGIN\ ENCRYPTED\ PRIVATE\ KEY-----/, 'PKCS#7 present');
