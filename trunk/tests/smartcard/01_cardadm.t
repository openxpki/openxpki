...
# which NAME, DIR ...
# returns the full path in which NAME was found
sub which {
    my $name = shift;
    my @dirs = @_;
...
if ( $ENV{DESTRUCTIVE_TESTS} ) {
    print "# purging and reloading LDAP data for tests\n";
    my $ldiffile = which( '01_cardadm.ldif', @cfgpath );
    ...
    warn "# Using LDAP config ",
        ...
        "\n";
    my @cmd = (
        $cfg{instance}{ldapdelete},              '-x', '-c', '-D',
        $cfg{'ldapadmin'}{'user'}, '-w', $cfg{'ldapadmin'}{'pass'},
        '-H', $cfg{'ldapadmin'}{'url'}, @dn
    );
    warn "# cmd: ", join(', ', @cmd), "\n";
    ...
    @cmd = (
        ...
        '-H',      $cfg{'ldapadmin'}{'url'},
        '-f', which( '01_cardadm.ldif', @cfgpath )
    );
    warn "# cmd: ", join(', ', @cmd), "\n";
...
SKIP: {
    ...
    $test->skip("LDAP with Token", 8) unless $cfg{tests}{ldaptok};
