...
my $passwd = $default_token->command ("create_random", RANDOM_LENGTH => 16);
ok (1);
print STDERR "passwd: $passwd\n" if ($ENV{DEBUG});
OpenXPKI->write_file (FILENAME => "$basedir/ca1/passwd.txt", CONTENT => $passwd);

## create DSA key
my $key = $default_token->command ("create_key",
                                   TYPE       => "DSA",
                                   KEY_LENGTH => "1024",
                                   ENC_ALG    => "aes256",
                                   PASSWD     => $passwd);
...
## create EC key
$key = $default_token->command ("create_key",
                                TYPE       => "EC",
...
my $csr = $default_token->command ("create_pkcs10",
                                   KEY     => $key,
                                   PASSWD  => $passwd,
                                   SUBJECT => $subject);
ok (1);
...
## create cert
my $cert = $ca_token->command ("issue_cert",
                               CSR     => $csr,
                               PROFILE => $profile);
ok (1);
...
my $pkcs12 = $default_token->command ("create_pkcs12",
                                      PASSWD  => $passwd,
                                      KEY     => $key,
                                      CERT    => $cert,
...
## $profile->set_serial (1);
my $crl = $ca_token->command ("issue_crl",
                              REVOKED => [$cert],
                              PROFILE => $profile);
