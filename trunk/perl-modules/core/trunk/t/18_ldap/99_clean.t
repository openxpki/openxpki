...
my $realm_config         = File::Spec->catfile(
				    $test_directory,
				    $test_group,
				    'ldappublic_test.xml',
		           );
my $realm_dummy_config   = File::Spec->catfile(
				    $test_directory,
				    $test_group,
				    'ldappublic_dummy.xml',
		           );
...
my @test_tasks = (
		    ...
		    '5 delete realm configuration',
		    '6 delete dummy realm configuration',
		);    
...
#### 5) DELETE REALM CONFIG
if (-f $realm_config){ unlink $realm_config; };
ok ( !(-f $realm_config),"Deleting realm configuration");

#### 6) DELETE REALM DUMMY CONFIG
if (-f $realm_dummy_config){ unlink $realm_dummy_config; };
ok ( !(-f $realm_dummy_config),"Deleting dummy realm configuration");
