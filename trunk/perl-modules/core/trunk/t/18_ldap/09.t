##
## CHECK NODE ERROR MESSAGES VALIDATION
##
...
# --------------------------------------- checking check_node error messages
#
# 1) calling check_node with a bad dn (wrong syntax)
# 2) calling check_node with a dn that does not exist
# 
#
...
#
  my $bad_dn_comments = [
    ' 1) calling check_node with a bad dn (wrong syntax)',
    ' 2) calling check_node while the node does not exist',
  ]; 
...
my $bad_dn_errors = [
                {
                  ...
                  'ACTION' => 'I18N_OPENXPKI_LDAPUTILS_CHECK_NODE_SIMPLE',
...
#-------------------- must fail to add and we will check reasons----------- Go
 ...
 for( my $i=0; $i < $test_number ; $i++ ) {
        $utils->reset_error;
        $utils->check_node( $ldap, $bad_dns->[$i] );
