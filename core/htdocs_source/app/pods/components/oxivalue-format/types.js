...
const types = {
    ...
    raw: v => v,
    defhash: v => `<dl>${Object.keys(v).map(k => `<dt>${k}</dt><dd>${Em.$('<div/>').text(v[k]).html()}</dd>`).join("")}</dl>`,
    deflist: v => `<dl>${Object.keys(v).map(k => `<dt>${v[k].label}</dt><dd>${(v[k].format === "raw" ? v[k].value : Em.$('<div/>').text(v[k].value).html())}</dd>`).join("")}</dl>`,
    ullist:  v => `<ul class="list-unstyled">${Object.keys(v).map(k => `<li>${Em.$('<div/>').text(v[k]).html()}</li>`).join("")}</ul>`,
