 ...
 *   - `format` { string }        - Optional: Button style override (default: `'tile'`)
 *   - `disabled` { boolean }     - Optional: Disable the button
 *   - `confirm` { object }       - Optional: Confirmation popup config (`label`, `description`,
 *     `confirm_label`, `cancel_label`)
 *   - `href` { string }          - Renders as `<a href>` link
 *   - `target` { string }        - Optional: Link target (only with `href`)
 *   - `page` { string }          - OpenXPKI page to load on click
 *   - `action` { string }        - OpenXPKI action to call on click
 *   - `action_params` { object } - Optional: Additional parameters sent with `action`
 *
