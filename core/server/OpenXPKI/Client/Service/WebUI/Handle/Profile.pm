package OpenXPKI::Client::Service::WebUI::Handle::Profile;
use Moose;
...
sub render_subject_form {
    ...
    #   uihandle: OpenXPKI::Client::Service::WebUI::Handle::Profile::render_subject_form!mode!renewal
    if ($param) {
    ...
    $self->log->trace("Additional parameters via 'uihandle' config: " . Dumper \%extra) if (scalar keys %extra and $self->log->is_trace);
    ...
    for my $field ($wf_info->{activity}->{$wf_action}->{field}->@*) {
        if (my $detected_section = $field2section{ $field->{name} }) {
            ...
            if (not $section) {
                $self->log->debug("Field '$parent_name' detected - setting profile section to '$detected_section'");
                $section = $detected_section;
            } else {
                if ($section ne $detected_section) {
                    $self->log->warn("Mismatch between section detected via field '$parent_name' and uihandle parameter: '$detected_section' != '$section'");
                }
            }
            last;
        }
    ...
    $self->log->debug("Render subject for '$parent_name', section '$section' in '$wf_action'");
    ...
    $self->log->trace('Presets: ' . Dumper $values) if $self->log->is_trace;
    ...
    foreach my $field (@{$profile_fields}) {
        ...
        $self->log->trace("Field '$name': transformed to web ui spec = " . Dumper $item) if $self->log->is_trace;
