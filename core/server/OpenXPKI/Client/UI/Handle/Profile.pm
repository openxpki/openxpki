...
sub render_profile_select {
    ...
    my @fields = (
        ...
        { name => "cert_subject_style", label => 'Subject Style', value => $cert_subject_style, type => 'select', 'options' =>[] },
    );
...
sub render_key_select {
    ...
    # Get the list of profiles from the backend - return is a hash with id => hash
    my $key_alg = $self->send_command( 'get_alg_names', {});
    # Transform hash into value/label list and sort it
    my @key_alg;
    foreach my $alg (sort keys %{$key_alg}) {
       push @key_alg, { label => $alg, value => $alg };
    }

    my $key_type;
    my @fields = (
        { name => "key_alg", label => 'Algorithm', value => $key_type, type => 'select', 'options' => \@key_alg, actionOnChange => 'profile!get_key_param' },
    );

    my $key_param_names = $self->send_command( 'get_param_names', {});

    # current values from context when changing values!
    my $context = $wf_info->{WORKFLOW}->{CONTEXT};
    my $key_param_values = $context->{key_param} ? $self->serializer()->deserialize( $context->{key_param} ) : {};

    foreach my $pn (keys %{$key_param_names}) {
        # We create the label as I18 string from the param name
        my $label = 'I18N_OPENXPKI_UI_KEYGEN_PARAM_'.$pn;
        push @fields, { name => "key_param{".lc($pn)."}", label => $label, value => $key_param_values->{ $pn }, type => 'select', 'options' => [] };
    }
