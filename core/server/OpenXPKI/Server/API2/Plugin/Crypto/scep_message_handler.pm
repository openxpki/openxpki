...
command "scep_unwrap_message" => {
    ...
    OpenXPKI::Exception->throw(
        message => 'Used recipient is not a valid ratoken',
        params => { identifier => $db_rcpt->[0]->{identifier} }
    ) unless($token_alias);
...
sub __generate_response {
    ...
    if ($mode eq 'success') {
        $req->certs([ $self->api->get_cert( identifier => $params->identifier, format => 'DER' ) ]);
