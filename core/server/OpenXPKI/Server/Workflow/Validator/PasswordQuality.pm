...
sub _init {
    ...
    if (exists $params->{checks}) {
        ...
        $self->enable($_) for @{$params->{checks}};
    } else {
        $self->enable(qw(
            common
            diffchars
    ...
    $self->enable('diffchars') and $self->mindiffchars($params->{mindiffchars}) if exists $params->{mindiffchars};

    # FIXME legacy parameter "dictionary"
    $self->enable('partdict')
        and $self->disable('dict')
        and $self->min_dict_len($params->{dictionary}) if exists $params->{dictionary};

    # FIXME legacy parameter "following"
    $self->enable('partsequence') and $self->sequence_len($params->{following}) if exists $params->{following};

    if ($self->is_enabled('dict') or $self->is_enabled('partdict')) {
        if ($self->first_existing_dict) {
            CTX('log')->application()->info("Using dictionary file " . $self->first_existing_dict);
        } else {
            CTX('log')->application()->warning("No dictionary found - skipping dictionary checks");
        }
    }
}
...
sub validate {
    ...
    if (not $self->password) {
        ...
        $self->error($self->check_length);
        $self->error($self->check_letters) if $self->is_enabled("letters");
        $self->error($self->check_digits) if $self->is_enabled("digits");
        $self->error($self->check_specials) if $self->is_enabled("specials");
        $self->error($self->check_mixedcase) if $self->is_enabled("mixedcase");
        $self->error($self->check_char_groups) if $self->is_enabled("groups");
        $self->error($self->check_sequence) if $self->is_enabled("sequence");
        $self->error($self->check_dict) if $self->is_enabled("dict");
        $self->error($self->check_common) if $self->is_enabled("common");
        $self->error($self->check_diffchars) if $self->is_enabled("diffchars");
        $self->error($self->check_partsequence) if $self->is_enabled("partsequence");
        $self->error($self->check_partdict) if $self->is_enabled("partdict");
    }
...
# Set the error or (without arguments) return the recorded errors.
#
# In list context returns the array with the error codes and the strings.
# In scalar context returns the concatenated error strings.
sub error {
    my ($self, $error) = @_;
    if ($error) {
