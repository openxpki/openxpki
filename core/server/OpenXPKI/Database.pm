package OpenXPKI::Database;
use OpenXPKI -class;

=head1 Name

OpenXPKI::Database - Handle database connections and encapsulate DB
...
# Project modules
use OpenXPKI::Database::Role::Driver;
use OpenXPKI::Database::QueryBuilder;
...
has 'query_builder' => (
    is => 'ro',
    isa => 'OpenXPKI::Database::QueryBuilder',
    lazy => 1,
    default => sub {
        my $self = shift;
        return OpenXPKI::Database::QueryBuilder->new(
            driver => $self->driver,
            sqlam => $self->sqlam,
...
sub _build_driver {
    ...
    OpenXPKI::Exception->throw (
        message => "Parameter 'type' missing: it must equal the last part of a package in the OpenXPKI::Database::Driver::* namespace.",
    ) unless $driver;
    delete $args{type};

    my $class = "OpenXPKI::Database::Driver::".$driver;
    ...
    OpenXPKI::Exception->throw (
        message => "Database driver class does not consume role OpenXPKI::Database::Role::Driver",
        params => { class_name => $class }
    ) unless $instance->does('OpenXPKI::Database::Role::Driver');
...
sub insert ($self, $arg) {
    ...
    for (keys %{ $arg->{values} }) {
        $arg->{values}->{$_} = $self->next_id($arg->{into})
            if (blessed $arg->{values}->{$_} and $arg->{values}->{$_}->isa("OpenXPKI::Database::AUTOINCREMENT")); # ::AUTOINCREMENT is a "virtual" package
    }
...
For more details see L<OpenXPKI::Database::Role::Driver>.
...
=pod
    ...
    '->| O:D::Driver::* |
       ...
       consumes
         ...
         .    '-------------------'
         .    .----------------------------.
         ....>| O:D::Role::CountEmulation  |
         .    '----------------------------'
         .    .----------------------------.    .------------------------------.
         ....>| O:D::Role::SequenceSupport | or | O:D::Role::SequenceEmulation |
         .    '----------------------------'    '------------------------------'
         .    .----------------------------.    .------------------------------.
         '...>| O:D::Role::MergeSupport    | or | O:D::Role::MergeEmulation    |
              '----------------------------'    '------------------------------'
...
=item * B<log> - Log object (anything that has C<info>, C<warn>, C<errror> etc. methods, required)
...
=item * B<driver> - database specific driver instance (consumer of L<OpenXPKI::Database::Role::Driver>)

=item * B<query_builder> - OpenXPKI query builder to create abstract SQL queries (L<OpenXPKI::Database::QueryBuilder>)

Usage:

    # returns an OpenXPKI::Database::Query object
...
For more informations see L<OpenXPKI::Database/insert>.
...
For more informations see L<OpenXPKI::Database/update>.
...
For more informations see L<OpenXPKI::Database/merge>.
...
For more informations see L<OpenXPKI::Database/delete>.
...
=item * B<$query> - query to run (either a I<OpenXPKI::Database::Query>
