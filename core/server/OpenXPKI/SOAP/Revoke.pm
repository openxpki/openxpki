...
my $configfile = $ENV{OPENXPKI_SOAP_CONFIG_FILE}
...
sub RevokeCertificate {
    ...
    $log->debug(
        "SOAP: Entered RevokeCertificate - ",
    ...
    if ( $reason !~
m{ \A ( unspecified | keyCompromise | CACompromise | affiliationChanged | superseded | cessationOfOperation | certificateHold | removeFromCRL ) \z }xms
      )
    {
        $reason = 'unspecified';
    }
  ...
  SERVICE_MESSAGE:
    while (1) {
        ...
        if ( $reply->{SERVICE_MSG} eq 'GET_AUTHENTICATION_STACK' ) {
            if ( defined $auth_stack ) {
                ...
                $log->error(
                    "Authentication stack requested but not configured");
                return SOAP::Data->new( name => 'responseCode', value => 1 );
            }
        }
    ...
    my %wf_parameters = (
        ...
        flag_batch_mode => 1,
        flag_auto_approval => $flag_requester_authorized,
