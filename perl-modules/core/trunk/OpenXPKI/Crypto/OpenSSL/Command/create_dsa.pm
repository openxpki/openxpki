...
sub get_command
{
    ...
    if ($self->{ENC_ALG} ne "aes256" and
        ...
        $self->{ENC_ALG} ne "des")
    {
        $self->set_error ("I18N_OPENXPKI_CRYPTO_OPENSSL_COMMAND_CREATE_DSA_WRONG_ENC_ALG");
        return undef;
    }
    ...
    {
        $self->set_error ("I18N_OPENXPKI_CRYPTO_OPENSSL_COMMAND_CREATE_DSA_WRONG_KEY_LENGTH");
        return undef;
    }
    if ($keyform ne "engine" and not defined $passwd)
    {
        ## missing passphrase
        $self->set_error ("I18N_OPENXPKI_CRYPTO_OPENSSL_COMMAND_CREATE_DSA_MISSING_PASSWD");
        return undef;
    }
